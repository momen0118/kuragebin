...
/**
 * 個体に触る操作。タップで札、長押しでカップに水ごと入れて運び、左右の端で隣の瓶の上へ、指を離すと注ぐ。
...
/**
 * カップ。薄い透明なプラスチックの計量カップ（飾りのない形）。海月を水ごと運んで、隣の瓶へ注ぐ。
...
export const SCOOP = {
  ...
  crossSeconds: 0.9,
  crossArc: 0.04,
  /** 注ぐ：口の上の注ぐ所へ動く時間、注ぎ口の瓶の軸からのずれ（来た側へ）、傾ける時間と最大の角度（ラジアン） */
  moveSeconds: 1.0,
  pourOffset: 0.05,
  tiltSeconds: 2.6,
  tiltMax: 2.1,
  ...
  leaveSeconds: 1.2,
  /**
   * 海月：カップの水の残り（割合）がこれより減ると、水と一緒に滑り出る。滑り出てから水面に入るまでの時間と、
   * そのうち注ぎ口まで出てくるのにかける割合。水に入ったあとの沈む速さ
   */
  exitFill: 0.3,
  slideSeconds: 1.8,
  slideLip: 0.2,
  /** 滑り降りたあと、水面から泳げる所まで沈むのにかける割合と、そのあとの沈む速さ */
  slideSink: 0.15,
  entrySpeed: 0.02,
  /** 注ぐ水の筋：いちばん勢いがあるときの太さ、落ちていく重さ（瓶の高さ/秒²）、注ぎ口から前へ出る速さ、勢いの目安（全部が出るまでの秒） */
  streamWidth: 0.018,
  streamGravity: 30,
  streamPush: 0.18,
  pourFlowSeconds: 1.2,
