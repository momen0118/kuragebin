...
export const SWIM = {
  ...
  othersPush: 0.06,
  othersDepth: 0.35,
  /** 指でつままれているとき、その先へ引かれるばね（1/秒²）と、水の抵抗（1/秒）。少し遅れてついてくる */
  holdSpring: 14,
  holdDamping: 5,
} as const;
...
/**
 * 個体に触る操作。タップで札、長押しでつまんで左右の端へ運ぶと隣の瓶へ移す。
...
export const HANDLING = {
  ...
  longPressMs: 450,
  /** つまんだとき、きゅっと縮む強さ（つついたときを 1 として） */
  grabStartle: 0.35,
  /** 画面の左右の端のこの幅（画面の幅に対する割合）に、つまんだ個体を運ぶと隣の瓶へ移す */
  edgeZone: 0.12,
  /** 端に運んでから移すまで（ミリ秒） */
  edgeDwellMs: 300,
  ...
  /** 移せなかったとき、瓶の真ん中のほうへ引き戻してから放すまで（秒） */
  returnSeconds: 0.9,
} as const;
...
export const TAG = {
  ...
  /** 個体の上に出すときの間（画面px） */
  offsetPx: 10,
  /** 名前を入れている間は、札をこの高さ（画面の高さに対する割合）より上に出す（キーボードに隠れないように） */
  editMaxY: 0.34,
} as const;
