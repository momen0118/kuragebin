...
**トーン**：暗く、静かで、遅い。色は黒・ダークグレー・ウォルナット。光源は画面外の窓から入る自然光と、夜の画面外のデスクライトだけ（発光する種を加えたあとは、その発光も）。
...
**画面に常にあるもの**：瓶、部屋、下端中央の小さな点3つ（今どの瓶か）、下の左右の隅の小さなアイコン（餌・日誌）。それ以外は出さない。夜の間だけ、照明のオン・オフの小さなアイコンを出す（昼・夕方は出さない）。

**アイコンの配置**：画面下の中央は瓶の点3つに使い、アイコン類は下の左右の隅に寄せる。ライトと餌は瓶に何かをする操作なので同じ側の隅に並べ、日誌は反対側の隅へ。ライトは夜だけ出る。

| 操作 | 結果 |
| --- | --- |
| 瓶の空いたところをタップ | ガラスをつつく。近くの海月がきゅっと縮んで離れ、数秒かけて緩む |
| 海月をタップ | その個体の小さな札が数秒だけ出る（名前・段階・この瓶に来てからの日数） |
| 餌アイコンをタップ | 水面から少量の餌が沈む。食べた個体は、胃のあたり（四つ葉の内側）に食べた餌がうっすら橙色に透けて見え、数時間かけて薄れて消える。成長がわずかに早まる。1瓶につき数時間に1回まで、それ以上は何も落ちない |
| 個体を長押し→左右の端へドラッグ | 隣の瓶へ移す。瓶が上限なら戻る |
| 日誌アイコンをタップ | 観察日誌を開く |
| 左右スワイプ | 瓶の切り替え。スライド中、隣の瓶の光が一瞬重なる |

設定（日誌の奥に置く）：音のオン・オフ、揺れを使うのオン・オフ、画質3段階、データの書き出し・読み込み。

名前は初期値なし。札に「名無し」と出て、タップで付けられる。

### 瓶を揺らす（フェーズ3で実装）

- 揺らす操作は端末そのものを動かすこと。DeviceMotion で端末の加速度と傾きを取る。スワイプなどの画面操作では揺らさない
- iOS は最初のタップで `DeviceMotionEvent.requestPermission()` を呼んで許可を取る。許可がない・対応しない端末では機能なしで普通に動くこと
- 傾き：水面は重力に対して水平を保とうとし、瓶に対して傾く。減衰する振り子程度の簡単なモデルでいい
- 揺らし：端末の加速度を瓶内の慣性力として、海月の本体・触手・口腕・マリンスノーに加える
- 傘は慣性で動き、触手は水と一緒に動く。瓶を急に下げたり、揺らして海月が転がったりしたときは、傘だけが先に動き、触手は水に取り残されて傘の上までふわっと舞い上がり広がる。そのあと数秒かけてゆっくり垂れ戻る。これが揺らしの一番の見どころなので、触手の水への追従を傘より強くしておく
- 強く振っても海月は傷つかない。力に上限を設け、水面の縁から小さな飛沫が跳ねる程度の演出に留める
- 揺らされた瓶のエフィラは成長が少し早まる（config.ts で調整、上限あり）。揺らさなくても不利益はない
- 設定に「揺れを使う」のオン・オフを追加する
- デバッグパネルに、センサーなしで揺れを試せる操作を追加する
...
- 最初のリリースはミズクラゲのみで飼育が成り立つようにする。別種はリリース後のアップデートで追加し、低確率で生まれる形にする。出現条件は追加するときに決める
- 追加の順は下の表のとおり
- 発光の仕組みは「種ごとに、いつも光るか・つついた直後だけ光るかを設定する」形にする

| 種 | 追加 | 特徴 |
| --- | --- | --- |
| ミズクラゲ | 最初のリリース | 半透明、四つ葉の模様。発光しない |
| Neoturris sp. | 第1弾 | 傘高約3cm。傘の縁・内部・触手が朱色 |
| Eirene menoni | 第1弾 | 傘径約2cm。お椀型の透明な傘、縁に真珠のような粒 |
| オワンクラゲ | 第1弾 | お椀型の透明な傘。つついたときだけ傘の縁に並んだ粒が青緑に光る。照明オフのときに一番よく見える |
| カブトクラゲ | 第2弾 | 厳密にはクシクラゲ。拍動せず、櫛板を虹色の光が流れる。虹は反射なので照明か窓の光がないと見えない |
| 目玉焼きクラゲ | 最後・余力があれば | |
| Rhizostoma pulmo | 最後・余力があれば | 瓶の中では若い姿のまま大きくならない。ごくまれ |

#### 個体の崩し（フェーズ4）

個体差を実装するときに、次を遺伝子に含める。どれも眺めているうちに気づく程度に。

- 傘の輪郭のわずかないびつさ
- 縁のところどころの欠けや、房（縁触手）の密度のむら
- 四つ葉の大きさ・位置の不揃い
- まれに三つ葉・五つ葉の個体
...
- 夜：窓光はほぼゼロ。画面外、瓶の左上（窓と同じ側）にあるクランプ式のアーム型デスクライトが主光源になる（ライト本体は映さない）。ヘッドが小さく光は狭い円錐で、左上から斜めに、瓶の開いた口と窓側のガラスから水中へ入る。天板には瓶を中心にやや右へずれた楕円の光だまりができ、輪郭はある程度はっきり切れて、その外は闇。瓶の影は昼と同じく右側へ落ちる。光の色は青みのある白（暖色だと水が茶色く濁って見える。青白いほうが海月の透明さが映える）。天板の光だまりも同じ色。瓶のガラスへの光の回り込みとマリンスノーの見え方は、この照明を光源にする（照明の円錐の中だけが見える）
- 照明は日の入り後に点き、日の出前に消える。昼・夕方は窓の光だけ。照明はオン・オフを切り替えられ、初期値はオン。オフにすると瓶はほぼ闇になる
...
- **発光**：ミズクラゲは本来発光しないので光らせない。夜は照明で見せる（輪郭・縁・放射管・生殖腺で照明の光が散って浮かぶ）。発光の仕組み（加算合成とブルーム）は残し、発光する種を後から足せるようにする。例：オワンクラゲは、つついたときだけ傘の縁の粒が青緑に光る。照明オフのときに一番よく見える
...
- **足すもの**：瓶の接地影、天板へのガラスの映り込み、夜はデスクライトの光だまりとその中の瓶の影。これらは手続き的に重ねる
...
3. **育つ瓶**：生活環（ポリプ・ストロビラ・エフィラ）、3瓶スワイプ、個体の移動、餌、観察日誌、瓶を揺らす
4. **仕上げ**：個体差と変異（個体の崩しを含む）、拾いもの、桜の季節、音（初期オフ）、画質設定
